...
6. Дождитесь сообщения `Setup complete` — установщик пропишет прокси и токен в `%USERPROFILE%\.claude\settings.json`.
...
**А)** `npx` не найден или `node --version` меньше `v20` — установите Node.js LTS:
...
**Б)** В браузере `403` / `access denied` на шаге device-кода — ваш GitHub-аккаунт не состоит в организации `sputnik-systems`. Напишите в `#dev`, чтобы вас добавили.
...
**Г)** `401 unauthorized` в ответе — токен протух, повторно запустите `asgardos-setup`.

**Д)** Ручной пошаговый путь — <https://github.com/sputnik-asgardos/llm-proxy/blob/main/CLAUDE_CODE_SETUP.md>.
